...
- Entry point: `Engine.ApplyTemplates` (`src/Dax.Template/Engine.cs`) reads `Configuration.Templates[]` and dispatches each entry by its `Class` string to a handler (`HolidaysDefinitionTable`, `HolidaysTable`, `CustomDateTable`, `MeasuresTemplate`, `CalendarTemplate`, `CalculationGroupTemplate`).
