...
public abstract class TableTemplateBase
{
    ...
    protected virtual void RenameWithTranslation(Table tabularTable, Translations.Language language, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var column in tabularTable.Columns)
        {
            ...
            if (columnTranslation is not null)
            {
        ...
        foreach (var measure in tabularTable.Measures)
        {
            ...
            if (measureTranslation is not null)
            {
        ...
        foreach (var hierarchy in tabularTable.Hierarchies)
        {
            ...
            if (hierarchyTranslation is not null)
            {
                ...
                foreach (var level in hierarchy.Levels)
                {
                    ...
                    if (levelTranslation is not null)
                    {
    ...
    private void SaveAffectedRelationships(Table tabularTable, CancellationToken cancellationToken = default)
    {
        ...
        if (tabularTable.Model is not null)
        {
            // Save relationships
            FixRelationshipsTo =
    ...
    {
        if (FixRelationshipsTo is not null)
        {
        ...
        if (FixRelationshipsFrom is not null)
        {
    ...
    protected virtual string GetSourceColumnName(Column column) => $"[{column.Name}]";

    protected virtual string? GetDefaultFormatString(Column column, Microsoft.AnalysisServices.Tabular.Model model) => null;
    ...
    protected virtual void AddColumns(Table dateTable, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            foreach (var column in Columns.Where(c => !c.IsTemporary))
            {
                ...
                if (column.AttributeType is not null)
                {
