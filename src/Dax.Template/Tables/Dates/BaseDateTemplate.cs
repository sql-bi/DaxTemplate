    ...
    protected override string? ProcessDaxExpression(string? expression, string lastStep, TabularModel? model = null, CancellationToken cancellationToken = default)
    {
        ...
        string GetDefaultVariable(string expression)
        {
            ...
            if (matchGetDefaultVariable.Success)
            {
                ...
                Config.DefaultVariables.TryGetValue(settingName, out string? replace);
                if (replace == null)
                {
        ...
        string GetYearEndFromFirstMonthVariable(string expression)
        {
            ...
            if (matchGetYearEnd.Success)
            {
                ...
                Config.DefaultVariables.TryGetValue(settingName, out string? firstMonth);
        ...
        {
            if (regexGetHolidayName.IsMatch(expression))
            {
        ...
        static string GetLastStep(string expression, string lastStep)
        {
            ...
            {
                if (regexGetLastStep.IsMatch(expression))
                {
        ...
        {
            if (regexGetMinDates.IsMatch(expression))
            {
        ...
        {
            if (regexGetMaxDates.IsMatch(expression))
            {
                string replace = "TODAY()";
                if (scanColumns != null)
                {
        ...
        {
            if (regexGetCalendar.IsMatch(expression))
            {
